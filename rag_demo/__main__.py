...
parser.add_argument('--skip-reload', action='store_true', help='Skip the reload process if this flag is provided.')
...
database_url = os.environ.get("DATABASE_URL", "postgresql://postgres:postgres@localhost:6432/rag_demo")
...
if not args.skip_reload:
    ...
    for filename in os.listdir(DATA_DIR):
        ...
        for chunk in split_string_by_length(content, CHUNK_SIZE):
            ...
            db.execute(
                "INSERT INTO items (embedding, chunk) VALUES (%s, %s)", [str(response.data[0].embedding), chunk]
            )
    db.commit()
...
result = db.execute("SELECT 1 - (embedding <=> %s) as score, chunk FROM items ORDER BY embedding <-> %s LIMIT 5", [embedding, embedding])
rows = list(result)
